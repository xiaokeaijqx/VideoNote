...
request.interceptors.request.use(config => {
  try {
    const password = localStorage.getItem('webAccessPassword')
    if (password) {
      config.headers = config.headers || {}
      config.headers['request-web-access-password'] = password
    }
  } catch {
    /* ignore */
  }
...
request.interceptors.response.use(
  ...
  error => {
    ...
    if (res) {
      // 如果后端有返回错误信息，则显示后端信息
      if (!suppress) toast.error(res.msg || res.detail || '服务器错误，请稍后再试')
