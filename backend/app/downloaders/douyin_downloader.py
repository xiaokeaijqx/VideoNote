...
class DouyinDownloader(Downloader):
    ...
    def extract_video_id(self, url: str) -> str:
        ...
        if len(video_url):
            ...
            try:
                ...
                return ""
        patterns = [
            r'video/(\d+)',                # https://www.douyin.com/video/{id}
            r'modal_id=(\d+)',             # https://www.douyin.com/jingxuan?modal_id={id} 、/discover?modal_id={id}
            r'note/(\d+)',                 # 图文笔记
            r'aweme_id=(\d+)',             # 内部跳转 / 分享后参数
        ]
    ...
    def fetch_video_info(self, video_url: str) -> json:
        try:
            ...
            if not aweme_id:
                raise ValueError(f"无法从抖音链接解析出 aweme_id：{video_url}")
