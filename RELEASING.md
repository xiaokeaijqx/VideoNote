...
git tag -a vX.Y.Z -m "VideoMemo vX.Y.Z
...
5. CI 跑完后 Release 页面会自动出现 `videomemo-extension-X.Y.Z.zip` / `.xpi` / `.crx`
...
2. 选 VideoMemo → 左侧 **Package** → **Upload new package**
3. 上传 `videomemo-extension-X.Y.Z.zip`
4. 检查 listing（描述 / 图标 / 截图无变化可保持），点 **Submit for review**

### Microsoft Edge Add-ons

1. https://partner.microsoft.com/dashboard/microsoftedge
2. 选 VideoMemo → **New submission**
...
2. 选 VideoMemo → **Upload New Version**
3. 上传 `videomemo-extension-X.Y.Z.xpi`
4. 选择"在 AMO 公开"或"自托管"
5. 提交审核
