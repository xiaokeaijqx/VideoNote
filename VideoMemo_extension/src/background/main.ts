...
onMessage<{ url: string; title?: string }, 'videomemo-start'>('videomemo-start', async ({ data, sender }) => {
...
browser.runtime.onInstalled.addListener(() => {
  console.log('VideoMemo extension installed')

  // 右键菜单：在视频页或视频链接上"用 VideoMemo 总结"
  try {
    browser.contextMenus.create({
      id: 'videomemo-summarize-page',
      title: '用 VideoMemo 总结此视频',
...
browser.contextMenus?.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== 'videomemo-summarize-page')
    return
