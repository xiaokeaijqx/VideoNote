...
- **AI 模型多模态能力开关**：模型配置新增「支持多模态」开关，默认关闭。未开启时，生成页会自动关闭截图 / 视频理解选项，后端也会把图片内容降级为文本提示，避免非多模态模型因 `image_url` 请求格式报错。
...
围绕「热点雷达 + 关键词订阅 + 推送通知」的一次大迭代（+4043 行 / 23 个文件）：

- **热点雷达（Trends）**：60+ 平台聚合展示（视频 / 新闻 / 财经 / IT 开发者四大类），支持拖拽排序、隐藏/恢复、用户自定义平台 ID，分类标签与标题同行；卡片右上角三点菜单新增「订阅」按钮，一键把当前热点收录到关键词订阅里。
- **关键词订阅（Subscriptions）**：精细化关键词语法——普通词模糊匹配、`+词` 必含、`-词` 排除（命中即丢弃整条）、`/正则/` 大小写不敏感；`any` / `all` 两种聚合模式；空关键词 = 订阅整平台（适合"全网热搜"型追踪）。
- **推送通知系统**：3 种通道——**Webhook**（任意 HTTP POST，支持 `{{title}}` / `{{body}}` / `{{url}}` 自定义 payload 模板）、**Bark**（iOS 推送神器，可选 `sound` / `group`）、**Email**（SMTP + STARTTLS，587 端口）；每条通道可独立启停、点"测试"按钮验证连通性。
- **定时调度器**：后端常驻线程，按 `TREND_CHECK_INTERVAL_MINUTES`（默认 30）拉一次全平台热点并自动匹配所有启用的订阅；匹配到新结果立即调用订阅绑定的推送通道；订阅页可一键"立即运行"。
- **「已订阅」标签页**：在「订阅」页多了一个 Tab，集中展示所有已启用订阅的最近匹配、命中关键词、未读数。
...
| **AI 视频笔记生成** | 基于 OpenAI 兼容大模型，支持精简 / 详细 / 教程 / 学术 / 小红书 / 会议纪要等多种风格，可选目录、原片跳转、关键画面截图；截图 / 视频理解会按模型多模态能力自动启停 |
...
| **热点雷达**（新） | 60+ 平台聚合展示（视频 / 新闻 / 财经 / IT 开发者），支持拖拽排序、隐藏/恢复、自定义平台 ID、分类标签；卡片三点菜单一键订阅 |
| **关键词订阅**（新） | 精细化关键词语法（`+必须` / `-排除` / `/正则/`）、`any` / `all` 模式、空关键词订阅全平台；订阅详情页查看匹配历史与未读数 |
| **推送通知**（新） | Webhook / Bark / Email 三通道，Webhook 支持 `{{title}}` / `{{body}}` / `{{url}}` payload 模板，每条通道可独立启停 + 测试发送 |
| **定时调度**（新） | 后端常驻线程，按 `TREND_CHECK_INTERVAL_MINUTES`（默认 30）拉全平台匹配订阅，命中即推送；订阅页可"立即运行"一次 |
...
1. **配置 AI 模型** —— 设置 → AI 模型设置 → 新建供应商（DeepSeek / OpenAI 兼容 / 本地都支持），填好 API Key，启用要用的模型。如模型支持图片输入，手动开启「支持多模态」；该开关默认关闭。
...
VideoMemo 内置了一套「热点订阅 → 关键词匹配 → 多通道推送」的工作流，让你不再被算法绑架，按自己的规则第一时间知道想看的热点。

### 1. 热点雷达（Trends）

左侧导航进入「热点雷达」，一屏看遍 60+ 平台的实时热门：

![热点雷达](./doc/screenshot-trend-radar.png)

- **四大分类**：
  - **视频** —— B 站 / 抖音 / 快手 / YouTube / 小红书
  - **新闻** —— 微博 / 知乎 / 百度 / 头条 / 澎湃 / 凤凰 / 贴吧 / 虎扑 / 联合早报 / 卫星通讯社 等
  - **财经** —— 华尔街见闻 / 财联社 / 36 氪 / 金十 / 雪球 / MKTNews / FastBull 等
  - **IT 开发者** —— IT 之家 / 掘金 / GitHub Trending / FreeBuf / AI 热搜 / Hacker News / Product Hunt / V2EX / 少数派 / 酷安 / 豆瓣 等
- **个性化布局**：拖拽卡片调整顺序、右上角关闭不想看的平台、随时点「恢复」找回；支持添加自定义平台 ID（任意 yt-dlp / RSS / API 数据源）。
- **去重 + 标签同行**：同名热点自动去重，分类标签与操作按钮（订阅 / 跳转）同行展示。

### 2. 关键词订阅（Subscriptions）

每个热点卡片右上角「···」→「订阅」即可创建一个订阅。**关键词语法**：

- **普通词** —— 不区分大小写的子串匹配
- **`+关键词`** —— 必须出现，否则整条丢弃
- **`-关键词`** —— 一旦出现整条丢弃（用于过滤广告 / 营销 / 无关内容）
- **`/正则/`** —— 标准正则，大小写不敏感

匹配模式可选 `any`（任一命中即匹配）或 `all`（全部命中才匹配）。**空关键词 = 订阅整平台**（"全网热搜"型追踪）。订阅页有「已订阅」Tab 集中展示命中历史、命中关键词、未读数。

![热点订阅](./doc/screenshot-trend-subscriptions.png)

### 3. 推送通道

设置页「通知通道」可建多组通道，独立配置独立测试：

| 通道 | 适用场景 | 关键配置 |
| --- | --- | --- |
| **Webhook** | 通用对接（飞书机器人 / 钉钉 / Discord / 自建服务） | URL + 可选 `template` 字段（支持 `{{title}}` / `{{body}}` / `{{url}}` 占位符渲染） |
| **Bark** | iOS 推送（无需自建后端） | device_key，可选 `sound` / `group` |
| **Email** | 邮件兜底（适合归档） | SMTP host / port（默认 587）/ 用户名 / 密码 / 收件人 |

新建通道后点「测试」按钮立即验证连通性；订阅里勾选「启用推送」并绑定通道 ID，匹配到新热点即自动推送（每订阅最多取前 10 条标题聚合正文）。

### 4. 定时调度

后端启动时拉起一个常驻线程（`TREND_SCHEDULER_ENABLED=false` 可关闭），按 `TREND_CHECK_INTERVAL_MINUTES`（默认 30）跑一次「全平台 → 全订阅 → 推送通道」完整链路。在订阅页点「立即运行」可手动触发一次，立即得到一次完整匹配结果，方便调试与首次配置验证。

## 🚀 一键推送飞书文档

把生成的笔记一键写入飞书云文档（docx），支持手动推送与生成后自动推送，既可落到云空间文件夹，也可直接归档进知识库（Wiki）。

![飞书推送](./doc/screenshot-feishu-push.png)

设置页「飞书推送」填好凭证即可启用：

- **基础凭证** —— 在飞书开放平台创建「企业自建应用」，拿到 **App ID / App Secret**；给应用开通 `docx:document`（创建/编辑文档）与 `drive:drive`（云空间）权限并发布版本。
- **目标文件夹 Token（可选）** —— 留空则推送到应用云空间根目录；填写后写入指定文件夹（地址 `/drive/folder/xxxx` 末尾即 token，需把应用加为该文件夹协作者）。
- **知识库节点链接 / Token（可选）** —— 填了就导入到知识库，笔记作为该节点下的子文档（需应用加入知识库并开通 `wiki` 权限）。填了知识库就优先走知识库，上面的文件夹仅作中转。
- **开放平台域名** —— 国内租户用 `https://open.feishu.cn`；海外 Lark 用 `https://open.larksuite.com`。
- **推送方式** —— `REST 直连`（直接调飞书开放平台导入接口，推荐最稳）/ `lark CLI` / `自动`。
- **启停与自动推送** —— 可单独「启用飞书推送」（关闭后笔记页「推送飞书」按钮仍可手动推送）；开启「生成后自动推送」则每篇笔记生成后自动写入飞书文档。点「测试连接」即时校验凭证，通过后即可在笔记页一键推送。
